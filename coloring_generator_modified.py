...
def parse_args():
    ...
    # 输出尺寸控制（不同模型支持不同方式）
    p.add_argument("--gemini-image-size", default="2K", choices=["1K", "2K", "4K"],
                   help="Gemini 系列 image_config.image_size（默认 2K）")
    p.add_argument("--openai-size", default=None, choices=["1024x1024", "1024x1536", "1536x1024", "auto"],
                   help="OpenAI GPT Image 的 size 覆盖值；不传则根据 orientation 自动选择")
